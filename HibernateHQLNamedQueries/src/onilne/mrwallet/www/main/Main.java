...
public class Main 
{
	...
	private static void retreiveDataIntoEntity(SessionFactory sessionFactory) {
		 ...
		 */
		Query query=session.createQuery(" FROM PERSON_ENTITY WHERE Id<? AND Id> :minId");
		query.setInteger(0, 5); // This is used to replace data at '?' where
								// first parameter defines the place qt which ?
								// is palced starting from 0
		query.setInteger("minId", 2);// This is used to replace the first
										// parameter name in query with second
										// parameter. Advantage here is that
										// order of placeholder need not be
										// remembered like in above case
		/*
